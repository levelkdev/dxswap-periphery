...
const { mkdir, readdir, readFile, writeFile, copyFile, rmdir } = promises;
...
const config = {
    paths: {
        outDir: join(ROOT, "bundle/dxswap-periphery"), // Path to root folder
...
const run = async () => {
    log(`DXswap-periphery contracts bundler`);
    log("=====================");
